...
if __name__ == '__main__':
    ...
    try:
        with open(os.path.join(output_dir, 'all.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(results['normal']))
        with open(os.path.join(output_dir, 'OAdH_ALL.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(results['strict']))
    except Exception as e:
    ...
    try:
        with open(os.path.join(output_dir, 'OAdH_NCR.txt'), 'w', encoding='utf-8') as f:
            f.write("\n".join(oadh_ncr_rules))
    except Exception as e:
        print(f"OAdH_NCR 写入失败：{str(e)}")
        sys.exit(1)
    
    # 更新文档
    update_readme(results['stats'], results['sources'], (oadh_ncr_rules, oadh_ncr_duplicates))
    print("规则处理完成！")
